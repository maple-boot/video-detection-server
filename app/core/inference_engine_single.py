    ...
    def detect(self, algorithm_id: str, frame: np.ndarray,
        ...
        try:
            ...
            if inference_time > 150:
                logger.warning(
                    f"推理耗时过高 | algorithm_id={algorithm_id} | "
                    f"time={inference_time:.1f}ms | detections={len(detections)} | "
                    f"tensorrt={is_tensorrt} | imgsz={imgsz}"
                )
            elif inference_time > 50:
                logger.debug(
                    f"detect | algorithm_id={algorithm_id} | "
                    f"time={inference_time:.1f}ms | detections={len(detections)} | "
                    f"tensorrt={is_tensorrt}"
                )
    ...
    def detect_sahi(self, algorithm_id: str, frame: np.ndarray,
        ...
        try:
            ...
            with self._lock:
                ...
                raw_infer_time = (time.time() - t_infer_start) * 1000
                inference_time = (time.time() - t0) * 1000
            # 映射坐标回原图
            all_detections = []
